...
// This should NOT be called during restore; we still mock it to ensure it isn't invoked.
  ...
  it('restores an existing session and returns brands + questions', async () => {
    ...
    if (result.success) {
      ...
      expect(result.data.questions).toEqual([
        { question: { logo: 'logo1.png' } },
        { question: { logo: 'logo2.png' } },
      ]);
    }

    // Restore should not persist anything or generate new questions
